...
public ArrayList<MathWord>SplitToMathWordsByOperands(String sExt){
	if(Consts.ToShowExprParsing)System.out.println("SplitToMathWordsByOperands starts working");
	...
	//LW=this.NO.size();//wa in prev vrn
	LW=this.NNS.size();
	if(Consts.ToShowExprParsing)System.out.println("Words prepared are:");//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	//first number is elaborated separately because q numbers is 1 > than  q operators, and all operators and numbers are elaborated in cycle
	//there is also other way - all them in cycle but for operators to check if i < LW, because LW operands exist, but LW-1 operators exists only
	/*
	...
	curMathWord.SetSmart(cs);//no atal do it in try or bef - anyway it is corrected in catch
	try{
	...
	////requires try catch
	//System.out.println("\n\nExpr has "+Integer.toString(expr.size())+ " mathematical words\n\n");
	...
	*/
	for(int i=1; i<=LW; i++){
		...
		//amda wa operator
		//
		//cs=s.substring(NNS.get(i+1-1)-1,NNF.get(i+1-1)+1-1);so wa in prev vrn now is
		cs=s.substring(NNS.get(i-1)-1,NNF.get(i-1)+1-1);
		Val=MathExpressionParsingLib.ArabNumCalc(MathExpressionParsingLib.ArabDigits(cs));
		curMathWord.SetSmart(cs);
		try{
			if(curMathWord.GetTypeN()==Consts.ArabianNumberTypeN && curMathWord.GetNumberValue()<Consts.ArabianMinVal){
		...
		//
		if(Consts.ToShowExprParsing)System.out.println("Number: "+cs+" = "+Integer.toString(Val)+ " == "+curMathWord.toString()+ " == "+expr.get(expr.size()-1).toString()+" Previous word = "+expr.get(expr.size()-1).toString());
		//^ operands
		//v operators
		if(i<LW){//Q operators s'1 < r'operands, so strictly < (atal uz nums, am S' <=)
	...
	if(Consts.ToShowExprParsing)System.out.println("SplitToMathWordsByOperands finishes working");
	return expr;
}//fn
/*
public ArrayList<MathWord>SplitToMathWordsByOperands(String sExt){
