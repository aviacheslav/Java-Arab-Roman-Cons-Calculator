...
public class MathExprSolver {
	public static CalcTransmitStruct Calc(ArrayList<MathWord>expr, int FromN){
		...
		//
		MathWord FirstWord=new MathWord();
		//
		int exprLen=expr.size();
		String operand1SpecStr;
		//
		FirstWord=expr.get(FromN-1);
	...
	//
	public static CalcTransmitStruct CalcPartialM(ArrayList<MathWord>expr, CalcTransmitStruct tr){
		CalcTransmitStruct R=new CalcTransmitStruct();
		int FromN=tr.FromN;
		...
		if(FromN>=1){
			...
			if((CurWord.toString()).equals("(")){
				//FromN++;
				tr.FromN=FromN+1;
				tr=CalcPartialM(expr, tr);
			}else if(CurWord.GetTypeN()==Consts.ArabianNumberTypeN || CurWord.GetTypeN()==Consts.RomanNumberTypeN){
				try{
					TheOperand1=(MathWord)CurWord.clone();
				}
			...
			//
			//TheOperator1=expr.get(FromN+1-1);
			CurWord=expr.get(FromN+1-1);
			if(!(CurWord.toString()).equals(")")){
				try{
					TheOperator1=(MathWord)CurWord.clone();
				}
				catch(CloneNotSupportedException ex){
					System.out.println(ex.getMessage());
					TheOperator1.SetSmart("NOP");
				} 
				...
				if((CurWord.toString()).equals("(")){
					...
					try{
						TheOperand2=(MathWord)CurWord.clone();
					}
					catch(CloneNotSupportedException ex){
						System.out.println(ex.getMessage());
						TheOperand2.SetSmart("0");
					} 
				}//else error
				//
				if(ArabianNotRoman){
					...
					TheOperand2.SetSmart(MathExpressionParsingLib.RomanNumberConstructor(TheOperand2.GetNumberValue()));
				}
				...
				if(operatorLevel2<=operatorLevel1 || FromN<=exprLen-2){
					...
					//try{
						...
						if(ArabianNotRoman){
							...
							operand1S=MathExpressionParsingLib.RomanNumberConstructor(operand1);//changed1
						}
						if(Consts.ToShowCalculating)System.out.println("operand1="+operand1S);
						...
						if(ArabianNotRoman){
							...
							TheOperand1.SetSmart(MathExpressionParsingLib.RomanNumberConstructor(operand1));
						}
						if(Consts.ToShowCalculating)System.out.println("operand1="+Integer.toString(TheOperand1.GetNumberValue()));//tried find error
						operand1=TheOperand1.GetNumberValue();
						if(Consts.ToShowCalculating)System.out.println("operand1="+Integer.toString(operand1));//tried find error
